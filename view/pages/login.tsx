...
const login : NextPage = () => {
    ...
    const submissionHandler= (values:any):void  => {
      ...
      axios.get('http://localhost:8000').then((res) => console.log(res)).catch((err:any) => console.log(err));
    };
    ...
    return (
       ...
       >
         <Form className="flex flex-col items-center justify-evenly  mt-40 text-white">
           ...
           <Field type="text" id="email" name="email" onChange={(e:any) => { user.email = e.target.value}} placeholder="email address" required className="bg-transparent py-2 border-b-2 placeholder-gray-300 focus:outline-none focus:bg-transparent"/>
           </div>
           <div className="flex flex-col items-center mb-10">
           <label className="hidden" htmlFor="password">Password</label>
           <Field type="password" id="password" name="password" onChange={(e:any) => { user.password = e.target.value}} placeholder="password" required className="bg-transparent py-2 border-b-2 placeholder-white focus:outline-none focus:bg-transparent"/>
           </div>           
